...
impl NodeFactory {
    fn new() -> Self {
        ...
        builders.insert(
            "table".to_string(),
            Box::new(DecisionTableNodeBuilder) as Box<dyn NodeBuilder>,
        );
        builders.insert(
            "expression".to_string(),
            Box::new(ExpressionNodeBuilder) as Box<dyn NodeBuilder>,
        );
        builders.insert(
            "function".to_string(),
            Box::new(FunctionNodeBuilder) as Box<dyn NodeBuilder>,
        );
        Self { builders }
    }
    ...
    fn build_edges(flow: &[Decision]) -> Vec<DecisionEdge> {
        flow.iter()
            .flat_map(|d| {
                d.sources
                    .iter()
                    .map(|source| DecisionEdge {
                        id: "".into(),
                        source_id: source.clone(),
                        target_id: d.id.clone(),
                        source_handle: Some("".into()),
                    })
                    .chain(d.targets.iter().map(|target| DecisionEdge {
                        id: "".into(),
...
impl DecisionGraphBuilder {
    ...
    pub fn build(&self, flow: Vec<Decision>) -> DecisionContent {
        let mut nodes = vec![DecisionNode {
        ...
        nodes.extend(
            flow.iter()
                .map(|d| self.node_factory.create_node(d.clone())),
        );
