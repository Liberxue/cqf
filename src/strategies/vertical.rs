...
impl<'a, T: OptionPricingModel> VerticalSpread<'a, T> {
    pub fn new(model: &'a T, s: f64, k1: f64, k2: f64, r: f64, sigma: f64, t: f64, is_bull: bool) -> Self {
        Self { model, s, k1, k2, r, sigma, t, is_bull }
    }
}
...
impl<'a, T: OptionPricingModel> OptionStrategy for VerticalSpread<'a, T> {
    fn price(&self) -> f64 {
        if self.is_bull {
            ...
            let p1 = self.model.put_price(self.s, self.k1, self.r, self.sigma, self.t);
            let p2 = self.model.put_price(self.s, self.k2, self.r, self.sigma, self.t);
            p1 - p2
        }
    }
}
